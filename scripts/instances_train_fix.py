...
# 1. 加载原始文件
with open("data/coco_lite10k/annotations/instances_train.json") as f:
    data = json.load(f)

# 2. 添加完整的80个COCO类别（与你的YAML配置一致）
data["categories"] = [
...
# 3. 保存修复后的文件
with open("data/coco_lite10k/annotations/instances_train_fixed.json", "w") as f:
